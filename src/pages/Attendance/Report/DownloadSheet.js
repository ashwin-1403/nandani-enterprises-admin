import axios from "axios";
import { ToastFailure, ToastSuccess } from "../../../Utils/Toast/ToastMsg";
import { formatDateForFilter } from "../../../Auth/schema";

export default async function DownloadSheet(
  url,
  search,
  verticalId,
  processId,
  stationId,
  startDate,
  endDate,
) {
  const startD =
    startDate === null ? "" : `${formatDateForFilter(startDate)} 00:00:00`;
  console.log(startD, "startD");
  const endD =
    endDate === null ? "" : `${formatDateForFilter(endDate)} 23:59:59`;
  try {
    await axios
      .get(
        `${process.env.REACT_APP_API}${url}?search=${search}&verticalId=${verticalId}&processId=${processId}&stationId=${stationId}&startDate=${startD}&endDate=${endD}`,
      )
      .then(() => {
        ToastSuccess("Success.");
        window.open(
          `${process.env.REACT_APP_API}${url}?search=${search}&verticalId=${verticalId}&processId=${processId}&stationId=${stationId}&startDate=${startD}&endDate=${endD}`,
          "_self",
        );
      });
  } catch (error) {
    ToastFailure(error?.response?.data?.message);
  }
}
