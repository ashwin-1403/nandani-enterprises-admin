...
import PayoutTable from "./PayoutList/PayoutTable";
...
// import DownloadSheet from "../Attendance/Report/DownloadSheet";

const pageLimit = 10;

function PayoutHeader() {
  const storedStartDate = sessionStorage.getItem("pay-startDate");
  const storedEndDate = sessionStorage.getItem("pay-endDate");
  const [loading, setLoading] = useState(false);
  const [startDate, setStartDate] = useState(
    storedStartDate
      ? getDateObject(sessionStorage.getItem("pay-startDate"))
      : null,
  );
  const [endDate, setEndDate] = useState(
    storedEndDate ? getDateObject(sessionStorage.getItem("pay-endDate")) : null,
  );
  const [currentPage, setCurrentPage] = useState(
    Number(sessionStorage.getItem("payoutCurrentPage")) || 1,
  );
  ...
  const [filterObj, setFilterObj] = useState({
    vertical:
      sessionStorage.getItem("payout-vertical") !== null
        ? sessionStorage.getItem("payout-vertical")
        : "",
    process:
      sessionStorage.getItem("payout-vertical") !== null
        ? sessionStorage.getItem("payout-process")
        : "",
    station:
      sessionStorage.getItem("payout-vertical") !== null
        ? sessionStorage.getItem("payout-station")
        : "",
  });
  ...
  const handleClearAll = () => {
    ...
    setStartDate(null);
    setEndDate(null);
    sessionStorage.removeItem("payout-vertical");
    sessionStorage.removeItem("payout-process");
    sessionStorage.removeItem("payout-station");
    sessionStorage.removeItem("pay-startDate");
    sessionStorage.removeItem("pay-endDate");
    sessionStorage.removeItem("payoutCurrentPage");
    setCurrentPage(1);
  };
  const handleChangeForVertical = (e) => {
    const { name, value } = e.target;
    setFilterObj((prev) => ({ ...prev, [name]: value, process: "" }));
    sessionStorage.setItem(`payout-${name}`, value);
    sessionStorage.setItem("payout-process", "");
    setCurrentPage(1);
  };
  ...
  const handleStartDateFilter = (date) => {
    ...
    if (endDate === null) {
      setEndDate(date);
      sessionStorage.setItem("pay-endDate", date.toISOString());
    }
    ...
    setStartDate(date);
    sessionStorage.setItem("pay-startDate", date.toISOString());
    setCurrentPage(1);
  };
  ...
  // const handlePayoutDownloadReport = async () => {
  //   DownloadSheet(
  //     "EmployeesPayOutReport",
  //     debouncedValue,
  //     filterObj.vertical,
  //     filterObj.process,
  //     filterObj.station,
  ...
  async function getListData() {
    try {
      ...
      const response = await GetApi(`payout/getpayouts`, params);
  ...
  const handleUpdatePay = async (userId) => {
    try {
      const start = startDate
        ? `${formatDateForFilter(startDate)} 00:00:00`
        : null;
      const end = endDate ? `${formatDateForFilter(endDate)} 23:59:59` : null;
      setLoading(true);
      const resUpdatePay = await PatchApi(
        `payout/update-payouts-status/${userId}?startDate=${start}&endDate=${end}`,
      );
      if (resUpdatePay?.status === 200) {
        setLoading(false);
        getListData();
        ToastSuccess(resUpdatePay?.message);
      }
    } catch (error) {
      console.log("error", error);
      setLoading(false);
  ...
  return (
    <div className="employeeSection attendanceEmployeeSection">
